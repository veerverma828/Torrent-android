...
public class PlayerActivity extends AppCompatActivity {
    ...
    private void emitProgress() {
        WritableMap data = baseEvent();
        data.putDouble("positionMs", player.getCurrentPosition());
        data.putDouble("durationMs", player.getDuration());
        emit("progress", data);
    }

    private WritableMap baseEvent() {
        WritableMap data = Arguments.createMap();
        data.putString("metadata", metadataJson);
        if (player != null) {
            data.putDouble("positionMs", player.getCurrentPosition());
            data.putDouble("durationMs", Math.max(0, player.getDuration()));
        }
        return data;
    }

    private void emit(String event, WritableMap data) {
        NativePlayerModule.emit(event, data);
    }
    ...
    private final Player.Listener playerListener = new Player.Listener() {
        ...
        public void onPlayerError(@NonNull PlaybackException error) {
            ...
            WritableMap data = baseEvent();
            data.putString("message", error.getMessage() != null ? error.getMessage() : "Playback error");
            emit("error", data);
        }
    };
